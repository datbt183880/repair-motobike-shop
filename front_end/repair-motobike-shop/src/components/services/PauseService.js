...
import "../../assets/css/services/pauseService.css";
...
function PauseService(props) {
  ...
  useEffect(() => {
    async function fetchServiceStillServingList() {
      try {
        ServicesService.getListServiceStillServing(filters).then((res) => {
    ...
    fetchServiceStillServingList();
  }, [filters]);

    const changeToStopServing = (id) =>{
        ServicesService.putServiceToStopServing(id)
        .then(() => {
    ...
    if (error) {
        ...
        return (
          <div className="list-pause-services">
            <div className="title-pause-services">
              <div className="name-title">
                <div className="button-cancel" onClick={back}>
                  <NavigateBefore style={{ width: "15px" }} /> <span>Quay lại</span>
                </div>
                <div className="name-list"><span>Danh sách dịch vụ đang phục vụ</span></div>
              </div>
            </div>
            ...
            <div className="content-pause-services">
              <Snackbars
                place="tc"
                color="info"
                message="Thành công!"
                    ...
                    {services.map((service) => (
                      <tr key={service.id}>
                        ...
                        <button
                            ...
                            onClick={() => changeToStopServing(service.id)}
                        >
                            <PauseIcon style={{ width: "15px" }} /><div  className="info-button"><span>Ngưng dịch vụ</span></div>
                        </button>
